<template>
    <div class="m-2" v-if="ready">
        <form @submit.prevent="submit">
            ...
            <div class="mb-4">
                <InputLabel for="matricula" value="Matricula" class="required" />
                <TextInput id="matricula" class="w-full" v-model="form.matricula" :disabled="readOnly"/>
                <InputError :message="form.errors.matricula" />
            </div>
            ...
            <div class="mb-4">
                <InputLabel for="status" value="Status" class="required" />
                <TextInput id="status" class="w-full" v-model="form.status" :disabled="readOnly"/>
                <InputError :message="form.errors.status" />
            </div>

            <div class="mb-4">
                <InputLabel for="valor_total" value="Valor Total" class="required" />
                <TextInput id="valor_total" class="w-full" v-model="form.valor_total" :disabled="readOnly"/>
...
function submit() {
    if (props.data?.id) {
        form.put(`/plano-contratacao/${props.data.id}`, {
            onSuccess: () => handleSuccess('Plano Contratação atualizado com sucesso!'),
            onError: () => handleError()
        });
    } else {
...
const loadData = async () => {
    try {
        const response = await axios.get(`/plano-contratacao/${props.data.id}`);
        // Set form data
